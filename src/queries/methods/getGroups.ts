...
import { GetGroupsByBoard, GetGroupsByBoardWithItems, GetGroupsByBoardWithItemsWithCells } from "../types/getGroups";
import { ClientOptions, QueryRequestOptions, QueryLevel, QueryNotCellRequestOptions, QueryCellRequestOptions } from "../../types/types";
...
async function getGroupsByBoardLevelGroup(
  ...
  if (!board) {
    throw new MonstaaError("query", `Board with board id: ${boardId} not found or you lack the necessary privileges to access this board.`);
  }

  return board.groups.map((group) => new Group(clientOptions, group.id, group.title, Number(board.id)));
}
async function getGroupsByBoardLevelItem(
  ...
  const result = await executeGraphQLQuery<GetGroupsByBoardWithItems>(clientOptions, requestOptions, query, variables);
...
async function getGroupsByBoardLevelCell(
  ...
  const query = requestOptions.columns ? GET_GROUPS_BY_BOARD_LEVEL_CELL : GET_GROUPS_BY_BOARD_LEVEL_CELL_ALL;
  ...
  const result = await executeGraphQLQuery<GetGroupsByBoardWithItemsWithCells>(clientOptions, requestOptions, query, variables);
...
export async function getGroupsByBoard(
  ...
  switch (queryLevel) {
    case QueryLevel.Board:
      throw new MonstaaError("query", `Query level chosen: ${queryLevel} is not applicable to the calling function: getGroupsByBoard.`);
    case QueryLevel.Group:
      return await getGroupsByBoardLevelGroup(clientOptions, boardId, requestOptions);
    case QueryLevel.Item:
      return await getGroupsByBoardLevelItem(clientOptions, boardId, requestOptions);
    case QueryLevel.Cell:
      return await getGroupsByBoardLevelCell(clientOptions, boardId, requestOptions);
    default:
