...
async function getGroupsByBoardLevelItem(
  ...
  return board.groups.map((group) => {
    const items = group.items_page.items.map(
      (item) =>
        new Item(clientOptions, item.id, item.name, group.id, Number(board.id))
    );
    return new Group(
      clientOptions,
      group.id,
...
export async function getGroupsByBoard(
  ...
  switch (queryLevel) {
    case QueryLevel.Board:
      throw new MonstaError(
        "query",
        `Query level chosen: ${queryLevel} is not applicable to the calling function: getGroupsByBoard.`
      );
