...
import {
  GET_SUBITEMS_BY_ITEM_LEVEL_CELL,
  GET_SUBITEMS_BY_ITEM_LEVEL_CELL_ALL,
  GET_SUBITEMS_BY_ITEM_LEVEL_SUBITEM,
} from "../strings/getSubitems";
import {
  GetSubitemsByItem,
  GetSubitemsByItemWithCells,
} from "../types/getSubitems";
...
async function getSubitemsByItemLevelSubitem(
  ...
  const result = await executeGraphQLQuery<GetSubitemsByItem>(
    clientOptions,
    requestOptions,
    query,
    variables
  ...
  if (!result.data.items[0]) {
    throw new MonstaError("query", `No item found with id: ${item.itemId}`);
  }
  const groupId = result.data.items[0].group.id;
  const boardId = result.data.items[0].board.id;

  return result.data.items[0].subitems.map(
    (item) =>
      new Subitem(clientOptions, item.id, item.name, groupId, Number(boardId))
  );
}

async function getSubitemsByItemLevelCell(
  clientOptions: ClientOptions,
  item: Item_SubitemQuery,
  ...
  const result = await executeGraphQLQuery<GetSubitemsByItemWithCells>(
    clientOptions,
    requestOptions,
    query,
    variables
  ...
  switch (queryLevel) {
    ...
    case QueryLevel.Group:
      throw new MonstaError(
        "query",
        `Query level chosen: ${queryLevel} is not applicable to the calling function: getSubitemsByItem.`
      );
    case QueryLevel.Item:
      return await getSubitemsByItemLevelSubitem(
        clientOptions,
        item,
        requestOptions
      );
    case QueryLevel.Cell:
      return await getSubitemsByItemLevelCell(
        clientOptions,
        item,
        requestOptions
      );
